...
def startup():
    ...
    try:
        ...
        print(f"[OK] Model loaded  ({_model_bundle['metrics']['model_type']})")
    except Exception as e:
        print(
            f"[ERROR] Model not found: {e}. Run preprocess_real.py then train_real.py first."
        )
    ...
    try:
        with open(MODEL_DIR / "meta.json") as f:
            _meta = json.load(f)
        print("[OK] Meta loaded")
    except Exception as e:
        print(f"[ERROR] Meta not found: {e}")
    ...
    try:
        ...
        print("[OK] Reference tables loaded")
    except Exception as e:
        print(f"[ERROR] Reference tables: {e}")


def _require_model():
    if not _model_bundle:
        raise HTTPException(
            503, "Model not loaded. Run preprocess_real.py then train_real.py first."
        )
...
class CompareDriverConfig(BaseModel):
    driver_id: int
    constructor_id: int
    year: int = 2024
    grid: int = 1
...
class CompareRequest(BaseModel):
    circuit_id: int
    circuit_length_km: float = 5.0
    laps: int = Field(30, ge=5, le=100)
    drivers: List[CompareDriverConfig]
...
def model_info():
    ...
    return {
        ...
        "y_std": m.get("y_std"),
        "top_features": [
            {"name": k, "importance": round(v, 5)} for k, v in top_features
        ],
    ...
    try:
        ...
        return {
            ...
            "features_used": {
                ...
                "tire_deg": round(
                    0.6 * (req.lap / req.total_laps)
                    + 0.4 * (req.lap / req.total_laps) ** 2,
                    4,
                ),
...
def compare(req: CompareRequest):
    """Simulate and compare multiple drivers (2 up to N) simultaneously in a single Grand Prix."""
    _require_model()
    try:
        PIT_TIME = 22.5
        driver_simulations = []

        for d_cfg in req.drivers:
            lap_results = []
            cum_time = 0.0
            for lap_num in range(1, req.laps + 1):
                row = _build_feature_row(
                    ...
                    d_cfg.year,
                    d_cfg.driver_id,
                    d_cfg.constructor_id,
                    req.circuit_id,
                )
                sec = _predict_time(row)
                is_pit = lap_num in [d_cfg.pit_lap_1, d_cfg.pit_lap_2]
                total_sec = sec + (PIT_TIME if is_pit else 0)
                cum_time += total_sec
            ...
            driver_simulations.append(
                {
                    "driver_id": d_cfg.driver_id,
                    "constructor_id": d_cfg.constructor_id,
                    "year": d_cfg.year,
                    "grid": d_cfg.grid,
                    "laps": lap_results,
                    "total_race_time": total_race_time,
                    "total_race_formatted": _fmt(total_race_time),
                    "fastest_lap_sec": round(min(pure), 3),
                    "fastest_lap_formatted": _fmt(min(pure)),
                    "avg_lap_time_sec": round(np.mean(pure), 3),
                    "avg_lap_formatted": _fmt(np.mean(pure)),
                    "pit_stops": [l for l in [d_cfg.pit_lap_1, d_cfg.pit_lap_2] if l],
                }
            )

        # Sort drivers by final total race time for classification & intervals
        driver_simulations.sort(key=lambda d: d["total_race_time"])
        leader_total = driver_simulations[0]["total_race_time"]

        for rank, d in enumerate(driver_simulations, start=1):
            d["position"] = rank
            gap = round(d["total_race_time"] - leader_total, 3)
            d["gap_to_leader"] = f"+{gap:.3f}s" if gap > 0 else "LEADER"

        # Build per-lap synchronized telemetry trace & gaps
        lap_traces = []
        for lap_idx in range(req.laps):
            lap_num = lap_idx + 1
            lap_entry = {"lap": lap_num}
            for d in driver_simulations:
                lap_info = d["laps"][lap_idx]
                lap_entry[f"driver_{d['driver_id']}_time"] = lap_info["lap_time_sec"]
                lap_entry[f"driver_{d['driver_id']}_cum"] = lap_info["cum_time_sec"]
            lap_traces.append(lap_entry)

        return {
            "circuit_id": req.circuit_id,
            "total_laps": req.laps,
            "classification": driver_simulations,
            "lap_traces": lap_traces,
            "winner_driver_id": driver_simulations[0]["driver_id"],
        }
    except Exception as e:
        raise HTTPException(500, str(e))
